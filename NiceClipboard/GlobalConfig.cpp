...
#define SETTING_ITEM(enabled, key, type, name, description) \
    { GlobalConfigManager::configKeyToString(key), QObject::tr(name), type, QObject::tr(description), g_defaultSettings[GlobalConfigManager::configKeyToString(key)], g_defaultSettings[GlobalConfigManager::configKeyToString(key)], enabled }

#define SETTING_ITEM_CB(enabled, key, type, name, description, callback, callbackData) \
    { GlobalConfigManager::configKeyToString(key), QObject::tr(name), type, QObject::tr(description), g_defaultSettings[GlobalConfigManager::configKeyToString(key)], g_defaultSettings[GlobalConfigManager::configKeyToString(key)], enabled, callback, callbackData }

#define SETTING_ITEM_EX(enabled, key, type, name, description, extraData) \
    { GlobalConfigManager::configKeyToString(key), QObject::tr(name), type, QObject::tr(description), g_defaultSettings[GlobalConfigManager::configKeyToString(key)], g_defaultSettings[GlobalConfigManager::configKeyToString(key)], enabled, SettingItem::ModifiedCallback(), QVariant(), extraData }

#define SETTING_ITEM_FULL(enabled, key, type, name, description, callback, callbackData, extraData) \
    { GlobalConfigManager::configKeyToString(key), QObject::tr(name), type, QObject::tr(description), g_defaultSettings[GlobalConfigManager::configKeyToString(key)], g_defaultSettings[GlobalConfigManager::configKeyToString(key)], enabled, callback, callbackData, extraData }
...
QList<SettingPage> g_settingPages{ {
    ...
    SETTING_PAGE("advanced", "高级", SETTING_ITEMS({
        ...
        SETTING_ITEM(true, GlobalConfigManager::MouseDoubleMiddleClickInterval, SettingItem::UInt32, "鼠标中键双击间隔：", "鼠标中键双击的时间间隔（毫秒）"),
        SETTING_ITEM(true, GlobalConfigManager::MouseDoubleRightClickInterval, SettingItem::UInt32, "鼠标右键双击间隔：", "鼠标右键双击的时间间隔（毫秒）"),
        SETTING_ITEM_EX(true, GlobalConfigManager::ClipboardHistoryListDragType, SettingItem::ComboBox, "历史记录列表拖动类型：", "剪贴板历史记录列表的拖动类型",
    ...
    SETTING_PAGE("shortcuts", "快捷键", SETTING_ITEMS({
        SETTING_ITEM(true, GlobalConfigManager::ShowWindowShortcut, SettingItem::KeySequence, "显示主窗口快捷键：", "用于显示主窗口的快捷键"),
    })),
    SETTING_PAGE("uiCustomization", "界面定制", SETTING_ITEMS({
        // 这里是界面定制的设置项，例如主题选择、字体选择等
        SETTING_ITEM(true, GlobalConfigManager::QSSMainWindowStyle, SettingItem::PlainTextEdit, "主窗口样式表：", "用于定制主窗口界面样式的QSS样式表文本"),
    })),
    SETTING_PAGE("about", "关于", SETTING_ITEMS({
        { ""/*这不是设置项，所以可以为空*/, QObject::tr("版本"), SettingItem::Custom, "软件版本", QVariant(), QVariant(), true, SettingItem::ModifiedCallback(), QVariant(),
